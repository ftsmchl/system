...
func (rs *RSSubCode) EncodeShards(pieces [][]byte) ([][]byte, error) {
	...
	*/
	...
	//we need to flatten the pieces into a byte slice
	data := make([]byte, uint64(len(pieces))*pieceSize)
	for i, piece := range pieces {
		//data = append(data, piece...)
		copy(data[uint64(i)*pieceSize:], piece)
	}
	fmt.Println("data before erasure coding is : ", string(data))

	erasuredPieces, err := rs.RSCode.Encode(data)
	if err != nil {
		fmt.Println("Something went wrong in erasure coding")
		return nil, err
	}

	for index, erPiece := range erasuredPieces {
		if index == 0 || index == 1 || index == 2 {
			fmt.Println("Erasured Piece : ", string(erPiece))
		} else {
			fmt.Println("Erasured Piece : ", erPiece)
		}
	}
	return erasuredPieces, nil
}
